...
class wsus_server_win::iis::config {
  ...
  iis_application_pool { 'WsusPool':
    name               => 'WsusPool',
    identity_type      => $wsus_server_win::identity_type,
    idle_timeout       => $wsus_server_win::idle_timeout,
    pinging_enabled    => $wsus_server_win::pinging_enabled,
    queue_length       => $wsus_server_win::queue_length,
    restart_time_limit => $wsus_server_win::restart_time_limit,
  }
  ...
  exec { 'WsusPoolPrivateMemoryLimit':
    ...
    command  => "\$privateMemoryPath = \"/system.applicationHost/applicationPools/add[@name=\'WsusPool\']/recycling/periodicRestart/@privateMemory\"
                 Set-WebConfiguration \$privateMemoryPath -Value ${$wsus_server_win::private_memory}",
