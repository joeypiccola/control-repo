...
Param(
    ...
    [string]$patterns,
    [Parameter(Mandatory = $True)]
    [ValidateSet('all_local_fixed_disks', 'user_specified_path')]
    [string]$search_type
)
...
switch ($search_type)
{
    'all_local_fixed_disks' {
        $paths = (Get-WmiObject win32_logicaldisk | Where-Object { $_.Description -eq 'Local Fixed Disk' }).DeviceID
    }
    'user_specified_path' {
        $paths = @()
        $paths += $path
...
$patternsArray = $patterns.Split(' ')
$searchStrings = @()

# test if provided path is valid
foreach ($pathItem in $paths) {
    if (-not (Test-Path -Path $pathItem)) {
        Write-Error "Provided path does not exist: `"$pathItem`"."
    }
}
...
Write-Output ($details | ConvertTo-STJson | Out-String -Width 1000000)
